...
void MainWindow::refreshUi(){
 if(!deviceList_)return;deviceList_->clear();for(const auto&t:runtime_->targets())deviceList_->addItem(QString("%1 · Android %2 · %3 · %4").arg(t.id,t.api,t.arch,t.state));
...
QWidget*MainWindow::makeSidebar(){auto*f=new QFrame;f->setObjectName("sidebar");f->setFixedWidth(240);auto*v=new QVBoxLayout(f);v->setContentsMargins(14,18,14,14);v->addWidget(lab("MobileLab","brand"));v->addWidget(lab("ANDROID · ARM64 LINUX","muted"));v->addSpacing(18);QStringList ns={"⌘  Overview","▦  Devices","▤  Test Matrix","◌  Live Console"};for(int i=0;i<ns.size();++i){auto*b=new QPushButton(ns[i]);b->setObjectName("nav");b->setProperty("selected",i==0);connect(b,&QPushButton::clicked,this,[this,i]{selectPage(i);});v->addWidget(b);}v->addStretch();auto*c=new QFrame;c->setObjectName("card");auto*x=new QVBoxLayout(c);x->addWidget(lab("ARM64 RUNTIME","cardTitle"));x->addWidget(lab(runtime_->architecture().contains("arm")?"● ARM64 native":"● Runtime detected","muted"));x->addWidget(lab(runtime_->kvmAvailable()?"KVM available":"KVM unavailable · userspace mode","muted"));x->addWidget(lab("REST 127.0.0.1:4000","muted"));v->addWidget(c);return f;}

QWidget*MainWindow::makeTopBar(){auto*f=new QFrame;f->setObjectName("toolbar");f->setFixedHeight(58);auto*h=new QHBoxLayout(f);h->setContentsMargins(16,8,16,8);h->addWidget(lab("Android Device Lab","title"));h->addStretch();connection_=lab(runtime_->kvmAvailable()?"● Connected · KVM":"● Connected · userspace ARM64","muted");h->addWidget(connection_);auto*newAvd=new QPushButton("＋ New AVD");newAvd->setObjectName("secondary");connect(newAvd,&QPushButton::clicked,this,&MainWindow::showAvdWizard);h->addWidget(newAvd);auto*b=new QPushButton("▶  Run Matrix");b->setObjectName("primary");connect(b,&QPushButton::clicked,this,&MainWindow::runMatrix);h->addWidget(b);return f;}

QWidget*MainWindow::makeDashboard(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);v->addWidget(lab("Overview","title"));v->addWidget(lab("Android orchestration and ARM64 execution on Linux","muted"));auto*c=new QHBoxLayout;c->addWidget(card("Targets",QString::number(runtime_->targets().size()),"discovered from Android SDK / AVD manager"));auto*a=card("Running","0","active emulator workloads");active_=a->findChild<QLabel*>("metric");c->addWidget(a);c->addWidget(card("Artifacts",artifacts_->root(),"logs and screenshots per run"));c->addWidget(card("API","4000","local REST endpoint"));v->addLayout(c);auto*s=new QSplitter(Qt::Horizontal);auto*l=new QFrame;l->setObjectName("card");auto*lv=new QVBoxLayout(l);lv->addWidget(lab("DEVICE POOL","cardTitle"));deviceList_=new QListWidget;lv->addWidget(deviceList_);s->addWidget(l);auto*r=new QFrame;r->setObjectName("card");auto*rv=new QVBoxLayout(r);rv->addWidget(lab("HOST CAPABILITIES","cardTitle"));rv->addWidget(lab("Architecture: "+runtime_->architecture(),"muted"));rv->addWidget(lab("Kernel: "+runtime_->kernel(),"muted"));rv->addWidget(lab(runtime_->kvmAvailable()?"KVM acceleration available":"KVM unavailable · no nested virtualization required by userspace ARM path","muted"));rv->addWidget(lab(emulator_->info().available?"Google Android Emulator: detected":"Google Android Emulator: not detected","muted"));rv->addWidget(lab(QString("Installed ARM images: %1").arg(emulator_->installedSystemImages().size()),"muted"));rv->addStretch();s->addWidget(r);v->addWidget(s,1);return p;}

QWidget*MainWindow::makeDevices(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);auto*h=new QHBoxLayout;h->addWidget(lab("Devices","title"));h->addStretch();auto*b=new QPushButton("＋ Create ARM64 AVD");b->setObjectName("primary");connect(b,&QPushButton::clicked,this,&MainWindow::showAvdWizard);h->addWidget(b);v->addLayout(h);v->addWidget(lab("Real AVD inventory from the Android Emulator / avdmanager toolchain","muted"));auto*l=new QListWidget;for(const auto&t:runtime_->targets())l->addItem(QString("%1     API %2     %3     %4").arg(t.id,t.api,t.arch,t.state));v->addWidget(l,1);return p;}

QWidget*MainWindow::makeMatrix(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);auto*h=new QHBoxLayout;h->addWidget(lab("Test Matrix","title"));h->addStretch();auto*b=new QPushButton("▶ Execute Matrix");b->setObjectName("primary");connect(b,&QPushButton::clicked,this,&MainWindow::runMatrix);h->addWidget(b);v->addLayout(h);v->addWidget(lab("Every selected AVD is scheduled, booted, validated, captured and released.","muted"));matrix_=new QTableWidget;matrix_->setColumnCount(5);matrix_->setHorizontalHeaderLabels({"Target","Android","ABI","Execution","Status"});matrix_->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);for(const auto&t:runtime_->targets()){const int r=matrix_->rowCount();matrix_->insertRow(r);matrix_->setItem(r,0,new QTableWidgetItem(t.id));matrix_->setItem(r,1,new QTableWidgetItem(t.api));matrix_->setItem(r,2,new QTableWidgetItem(t.arch));matrix_->setItem(r,3,new QTableWidgetItem(runtime_->kvmAvailable()?"accelerated/native":"userspace ARM64"));matrix_->setItem(r,4,new QTableWidgetItem("READY"));}v->addWidget(matrix_,1);return p;}

QWidget*MainWindow::makeConsole(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);v->addWidget(lab("Live Console","title"));v->addWidget(lab("Runtime, scheduler, matrix and provisioning events","muted"));console_=new QTextEdit;console_->setReadOnly(true);console_->setFont(QFont("monospace",10));console_->setPlainText(QString("[MobileLab] Android ARM64 framework initialized.\n[probe] arch=%1\n[probe] KVM=%2\n[probe] emulator=%3\n[probe] installed ARM images=%4\n[api] REST listening on 127.0.0.1:%5").arg(runtime_->architecture(),runtime_->kvmAvailable()?"available":"unavailable",emulator_->info().available?"available":"unavailable").arg(emulator_->installedSystemImages().size()).arg(api_->port()));v->addWidget(console_,1);return p;}

QWidget*MainWindow::makeInspector(){auto*f=new QFrame;f->setObjectName("inspector");f->setFixedWidth(280);auto*v=new QVBoxLayout(f);v->setContentsMargins(14,18,14,14);v->addWidget(lab("Inspector","title"));v->addWidget(lab("RUNTIME","cardTitle"));auto*c=new QFrame;c->setObjectName("card");auto*x=new QVBoxLayout(c);x->addWidget(lab("Android ARM64 Runtime","brand"));x->addWidget(lab("Linux / ARM64","muted"));x->addWidget(lab(runtime_->qemuAvailable()?"QEMU: available":"QEMU: not installed","muted"));x->addWidget(lab(emulator_->info().available?"Google Emulator: available":"Google Emulator: not installed","muted"));v->addWidget(c);v->addWidget(lab("ACTIONS","cardTitle"));auto*run=new QPushButton("Start Selected Target");run->setObjectName("secondary");connect(run,&QPushButton::clicked,this,[this]{if(!runtime_->targets().isEmpty())runtime_->start(runtime_->targets().first().id);});v->addWidget(run);auto*stop=new QPushButton("Stop Selected Target");stop->setObjectName("secondary");connect(stop,&QPushButton::clicked,this,[this]{if(!runtime_->targets().isEmpty())runtime_->stop(runtime_->targets().first().id);});v->addWidget(stop);auto*shell=new QPushButton("Run ARM64 Shell Probe");shell->setObjectName("secondary");connect(shell,&QPushButton::clicked,this,[this]{if(!runtime_->targets().isEmpty())runtime_->shell(runtime_->targets().first().id,"getprop ro.product.cpu.abi");});v->addWidget(shell);v->addStretch();return f;}

void MainWindow::showAvdWizard(){
 QDialog d(this);d.setWindowTitle("Create ARM64 Android AVD");d.resize(560,300);auto*v=new QVBoxLayout(&d);v->addWidget(lab("New ARM64 Android Device","title"));v->addWidget(lab("Provision through Google's Android SDK / avdmanager. No system images are bundled in MobileLab.","muted"));auto*grid=new QGridLayout;auto*name=new QLineEdit("mobilelab-arm64-api35");auto*pkg=new QComboBox;auto catalog=AndroidPackageCatalog(emulator_->sdkRoot());for(const auto&p:catalog.systemImages())pkg->addItem(p.displayName,p.id);if(pkg->count()==0)pkg->addItem("No installed ARM64 system images detected","system-images;android-35;default;arm64-v8a");auto*device=new QLineEdit("pixel_8");grid->addWidget(lab("AVD name","muted"),0,0);grid->addWidget(name,0,1);grid->addWidget(lab("System image","muted"),1,0);grid->addWidget(pkg,1,1);grid->addWidget(lab("Device profile","muted"),2,0);grid->addWidget(device,2,1);v->addLayout(grid);auto*info=lab(QString("SDK: %1\nEmulator: %2\nKVM: %3").arg(emulator_->sdkRoot(),emulator_->info().available?"detected":"not detected",runtime_->kvmAvailable()?"available":"unavailable"),"muted");v->addWidget(info);auto*buttons=new QDialogButtonBox(QDialogButtonBox::Cancel|QDialogButtonBox::Ok);buttons->button(QDialogButtonBox::Ok)->setText("Create AVD");connect(buttons,&QDialogButtonBox::accepted,&d,&QDialog::accept);connect(buttons,&QDialogButtonBox::rejected,&d,&QDialog::reject);v->addWidget(buttons);if(d.exec()==QDialog::Accepted){if(emulator_->createAvd(name->text(),pkg->currentData().toString(),device->text())){runtime_->refreshTargets();refreshUi();if(console_)console_->append("[avd] created "+name->text());}else QMessageBox::warning(this,"AVD creation failed","avdmanager could not create the requested AVD. Check that the selected ARM64 system image is installed.");}}

void MainWindow::runMatrix(){
 QVector<MatrixTarget> targets;for(const auto&t:runtime_->targets())if(t.id!="arm64-dev")targets.push_back({t.id,t.api,t.arch,"android-test"});
 if(targets.isEmpty()){QMessageBox::information(this,"No AVDs","Create or install an ARM64 AVD first, then run the matrix.");return;}
 selectPage(2);if(console_)console_->append(QString("[matrix] executing %1 ARM64 target(s)").arg(targets.size()));matrixExecutor_->run(targets);if(console_)console_->append("[matrix] artifacts: "+matrixExecutor_->lastRun().value("artifactDir").toString());
}
