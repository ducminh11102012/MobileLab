...
QWidget*MainWindow::makeDevices(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);auto*h=new QHBoxLayout;h->addWidget(lab("Devices","title"));h->addStretch();auto*b=new QPushButton("＋ Create Hybrid AVD");b->setObjectName("primary");connect(b,&QPushButton::clicked,this,&MainWindow::showAvdWizard);h->addWidget(b);v->addLayout(h);v->addWidget(lab("Real AVD inventory from Android Emulator / avdmanager, annotated by ABI stability","muted"));auto*l=new QListWidget;for(const auto&t:runtime_->targets())l->addItem(QString("%1     API %2     %3     %4     %5").arg(t.id,t.api,t.arch,t.state,t.stability));v->addWidget(l,1);return p;}

QWidget*MainWindow::makeMatrix(){auto*p=new QWidget;auto*v=new QVBoxLayout(p);v->setContentsMargins(22,22,22,18);auto*h=new QHBoxLayout;h->addWidget(lab("Test Matrix","title"));h->addStretch();auto*b=new QPushButton("▶ Execute Matrix");b->setObjectName("primary");connect(b,&QPushButton::clicked,this,&MainWindow::runMatrix);h->addWidget(b);v->addLayout(h);v->addWidget(lab("Selected x86_64 and ARM64 targets are scheduled together; ARM64 is marked fundamental until package availability and compatibility improve.","muted"));matrix_=new QTableWidget;matrix_->setColumnCount(5);matrix_->setHorizontalHeaderLabels({"Target","Android","ABI","Execution","Status"});matrix_->horizontalHeader()->setSectionResizeMode(QHeaderView::Stretch);for(const auto&t:runtime_->targets()){const int r=matrix_->rowCount();matrix_->insertRow(r);matrix_->setItem(r,0,new QTableWidgetItem(t.id));matrix_->setItem(r,1,new QTableWidgetItem(t.api));matrix_->setItem(r,2,new QTableWidgetItem(t.arch));matrix_->setItem(r,3,new QTableWidgetItem(runtime_->kvmAvailable()?"accelerated hybrid":"degraded userspace"));matrix_->setItem(r,4,new QTableWidgetItem("READY"));}v->addWidget(matrix_,1);return p;}
